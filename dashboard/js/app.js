...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (btnShowMetrics) {
        btnShowMetrics.onclick = async () => {
            ...
            try {
                const response = await fetch(`http://localhost:8000/api/metrics?token=${jwtToken}`);
                const data = await response.json();
    ...
    if (btnExportClients) {
        btnExportClients.onclick = () => {
            window.open(`http://localhost:8000/api/export/clients?token=${jwtToken}`, '_blank');
        };
    }
    
    if (btnExportHistory) {
        btnExportHistory.onclick = () => {
            window.open(`http://localhost:8000/api/export/history?token=${jwtToken}`, '_blank');
        };
    }
    ...
    function conectarServidor() {
        ws = new WebSocket(`ws://localhost:8000/ws/chat/painel_${attendantName}?token=${jwtToken}`);
    ...
    async function abrirChatNaTela(sessaoId) {
        ...
        if (!chats[sessaoId].historicoCarregado) {
            try {
                const response = await fetch(`http://localhost:8000/api/history/${sessaoId}`);
                if (response.ok) {
